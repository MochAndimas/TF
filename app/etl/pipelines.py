...
class GoogleSheetApi(DateWindowPipelineRunner):
    ...
    async def _fetch_instagram_insights(
        ...
        start_date,
        end_date,
        cached_follow_activity: dict | None = None,
        cached_media_totals: dict | None = None,
    ) -> list[dict]:
        """Fetch raw Instagram daily insight metrics for the requested ETL window."""
        return await self.extractor.fetch_instagram_insights(
    ...
    async def _fetch_youtube_daily_insight(self, start_date, end_date) -> list[dict]:
        ...
        return await self.extractor.fetch_youtube_daily_insight(
            start_date=start_date,
            end_date=end_date,
        )

    async def _fetch_youtube_media_insight(self, start_date, end_date) -> list[dict]:
        """Fetch YouTube per-content snapshots for the requested publish window."""
        return await self.extractor.fetch_youtube_media_insight(
            start_date=start_date,
            end_date=end_date,
        )
    ...
    async def _fetch_facebook_page_media_insights(self, start_date, end_date) -> list[dict]:
        """Fetch raw Facebook Page post/media insight metrics for the requested ETL window."""
        return await self.extractor.fetch_facebook_page_media_insights(start_date=start_date, end_date=end_date)
    ...
    @staticmethod
    def _parse_youtube_media_insight_dataframe(raw_rows: list[dict]):
        """Parse raw YouTube media snapshots into a normalized dataframe."""
        return parse_youtube_media_insight_dataframe(raw_rows)
    ...
    @staticmethod
    def _parse_facebook_page_media_insights_dataframe(raw_rows: list[dict]):
        """Parse raw Facebook Page media insights into a normalized dataframe."""
        return parse_facebook_page_media_insights_dataframe(raw_rows)
    ...
    def _build_youtube_daily_insight_models(df, pull_date):
        """Convert validated YouTube daily metrics into load payload rows."""
        return build_youtube_daily_insight_rows(df=df, pull_date=pull_date)

    @staticmethod
    def _build_youtube_media_insight_models(df, pull_date):
        """Convert validated YouTube media snapshots into load payload rows."""
        return build_youtube_media_insight_rows(df=df, pull_date=pull_date)
    ...
    @staticmethod
    def _build_facebook_page_media_insights_models(df, pull_date):
        """Convert validated Facebook Page media insights dataframe into load payload rows."""
        return build_facebook_page_media_insights_rows(df=df, pull_date=pull_date)
        ...
        async def extract(target_start, target_end):
            ...
            existing_rows = await session.execute(
                select(
                    ...
                    InstagramInsights.new_followers,
                    InstagramInsights.unfollowers,
                ).where(
                    InstagramInsights.date.between(target_start, refresh_from - timedelta(days=1))
                )
            )
            cached_follow_activity = {
                row.date: {
                    "followers": int(row.new_followers or 0),
                    "unfollowers": int(row.unfollowers or 0),
                }
                for row in existing_rows
            }
            media_totals_result = await session.execute(
                select(
                    InstagramMediaInsights.date,
                    func.sum(InstagramMediaInsights.likes).label("likes"),
                    func.sum(InstagramMediaInsights.comments).label("comments"),
                    func.sum(InstagramMediaInsights.shares).label("shares"),
                    func.sum(InstagramMediaInsights.saves).label("saves"),
                )
                .where(InstagramMediaInsights.date.between(target_start, target_end))
                .group_by(InstagramMediaInsights.date)
            )
            cached_media_totals = {
                row.date: {
                ...
                for row in media_totals_result
            }
            return await self._fetch_instagram_insights(
    ...
    async def youtube_daily_insight(
        self,
        session: AsyncSession,
        start_date=None,
        end_date=None,
        ...
        """Run YouTube Analytics ETL flow into ``youtube_daily_insight``."""

        async def extract(target_start, target_end):
            return await self._fetch_youtube_daily_insight(
                start_date=target_start,
                end_date=target_end,
            )

        async def stage(session_: AsyncSession, raw_rows: list, run_id_: str | None) -> int:
            return await stage_youtube_daily_insight_raw(
                session=session_,
                raw_rows=raw_rows,
                run_id=run_id_,
                source="youtube_daily_insight",
            )

        async def delete_window(session_: AsyncSession, target_start, target_end) -> int:
            return await delete_rows_in_date_window(
                session=session_,
                model_cls=YouTubeDailyInsight,
                window_start=target_start,
                window_end=target_end,
            )

        spec = DateWindowPipelineSpec(
            label="youtube_daily_insight",
            source="youtube_daily_insight",
            empty_metric_name="YouTube daily insight",
            date_column="date",
            auto_skip_model=YouTubeDailyInsight,
            extract=extract,
            stage=stage,
            parse=self._parse_youtube_daily_insight_dataframe,
            validate=validate_youtube_daily_insight_dataframe,
            build_rows=self._build_youtube_daily_insight_models,
            delete_window=delete_window,
            load_rows=upsert_youtube_daily_insight_rows,
        )
        return await self._run_date_window_pipeline(
            spec=spec,
            session=session,
    ...
    async def youtube_media_insight(
        self,
        session: AsyncSession,
        start_date=None,
        end_date=None,
        ...
        """Run YouTube per-content snapshot ETL into ``youtube_media_insight``."""

        async def extract(target_start, target_end):
            return await self._fetch_youtube_media_insight(
                start_date=target_start,
                end_date=target_end,
            )

        async def stage(session_: AsyncSession, raw_rows: list, run_id_: str | None) -> int:
            return await stage_youtube_media_insight_raw(
                session=session_,
                raw_rows=raw_rows,
                run_id=run_id_,
                source="youtube_media_insight",
            )

        async def delete_window(session_: AsyncSession, target_start, target_end) -> int:
            return await delete_rows_in_date_window(
                session=session_,
                model_cls=YouTubeMediaInsight,
                window_start=target_start,
                window_end=target_end,
            )

        spec = DateWindowPipelineSpec(
            label="youtube_media_insight",
            source="youtube_media_insight",
            empty_metric_name="YouTube media insight",
            date_column="date",
            auto_skip_model=YouTubeMediaInsight,
            extract=extract,
            stage=stage,
            parse=self._parse_youtube_media_insight_dataframe,
            validate=validate_youtube_media_insight_dataframe,
            build_rows=self._build_youtube_media_insight_models,
            delete_window=delete_window,
            load_rows=upsert_youtube_media_insight_rows,
        )
        return await self._run_date_window_pipeline(
            spec=spec,
            session=session,
    ...
    async def facebook_page_media_insights(
        self,
        session: AsyncSession,
        start_date=None,
        end_date=None,
        ...
        """Run Facebook Page post/media Insights ETL into ``facebook_page_media_insights``."""
        async def extract(target_start, target_end):
            return await self._fetch_facebook_page_media_insights(start_date=target_start, end_date=target_end)

        async def stage(session_: AsyncSession, raw_rows: list, run_id_: str | None) -> int:
            return await stage_facebook_page_media_insights_raw(
                session=session_,
                raw_rows=raw_rows,
                run_id=run_id_,
                source="facebook_page_media_insights",
            )

        async def delete_window(session_: AsyncSession, target_start, target_end) -> int:
            return await delete_rows_in_date_window(
                session=session_,
                model_cls=FacebookPageMediaInsights,
                window_start=target_start,
                window_end=target_end,
            )

        spec = DateWindowPipelineSpec(
            label="facebook_page_media_insights",
            source="facebook_page_media_insights",
            empty_metric_name="Facebook Page media insights",
            date_column="date",
            auto_skip_model=FacebookPageMediaInsights,
            extract=extract,
            stage=stage,
            parse=self._parse_facebook_page_media_insights_dataframe,
            validate=validate_facebook_page_media_insights_dataframe,
            build_rows=self._build_facebook_page_media_insights_models,
            delete_window=delete_window,
            load_rows=upsert_facebook_page_media_insights_rows,
        )
        return await self._run_date_window_pipeline(
            spec=spec,
            session=session,
