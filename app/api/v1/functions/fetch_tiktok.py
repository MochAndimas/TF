...
async def fetch_tiktok_analytics_payload(
    ...
    *,
    ...
    previous_media_df = await _read_media_rows(session=session, start_date=previous_start, end_date=previous_end)
    media_summary = media_summary_with_growth(
        _media_summary(media_df), _media_summary(previous_media_df),
        start_date=start_date, end_date=end_date, previous_start=previous_start, previous_end=previous_end,
    )
    current_metrics = _snapshot_summary(daily_df)["current_period"]["metrics"]
    previous_metrics = _snapshot_summary(previous_df)["current_period"]["metrics"]
    account_metrics = {
    ...
    return {
        "revenue": await fetch_socmed_revenue(session, source="tiktok", start_date=start_date, end_date=end_date, comparison=revenue_comparison),
