...
@router.get("/api/campaign/leads-by-source")
async def leads_by_source(
    ...
    """Return leads-by-source analytics payload in Plotly JSON format."""
    try:
        ...
        data = await fetch_leads_by_source_payload(
        ...
        return JSONResponse(
            content={
                "success": True,
                "message": "Leads by source analytics generated.",
                "data": data,
            }
        )
        ...
        raise HTTPException(
            ...
            detail=f"An error occurred while generating campaign analytics: {error}",
        )


@router.get("/api/campaign/overview")
