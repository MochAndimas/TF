...
class OverviewLeadsAcquisitionData:
    ...
    async def _read_one_source_ads_ua(self, model, source_key: str, from_date: date, to_date: date, ad_type: str = "user_acquisition") -> pd.DataFrame:
        query = (
            select(
            ...
            .join(model.campaign)
            .where(func.date(model.date).between(from_date, to_date), Campaign.ad_type == ad_type)
    ...
    async def _read_ads_ua_with_range(self, from_date: date, to_date: date) -> pd.DataFrame:
        return await self._read_ads_by_ad_type_with_range("user_acquisition", from_date, to_date)

    async def _read_ads_by_ad_type_with_range(self, ad_type: str, from_date: date, to_date: date) -> pd.DataFrame:
        frames = [
            await self._read_one_source_ads_ua(GoogleAds, "google", from_date, to_date, ad_type=ad_type),
            await self._read_one_source_ads_ua(FacebookAds, "facebook", from_date, to_date, ad_type=ad_type),
            await self._read_one_source_ads_ua(TikTokAds, "tiktok", from_date, to_date, ad_type=ad_type),
        ]
    ...
    async def _read_revenue_ua_with_range(self, from_date: date, to_date: date) -> pd.DataFrame:
        return await self._read_first_deposit_revenue_with_range(from_date, to_date, ad_type="user_acquisition")

    async def _read_first_deposit_revenue_with_range(self, from_date: date, to_date: date, ad_type: str) -> pd.DataFrame:
        query = (
            select(DataDepo.tanggal_regis.label("date"), func.sum(DataDepo.first_depo).label("revenue"))
            .join(DataDepo.campaign)
            .where(DataDepo.tanggal_regis.between(from_date, to_date), Campaign.ad_type == ad_type, DataDepo.first_depo.is_not(None), DataDepo.first_depo > 0)
    ...
    async def _cost_to_revenue_frames(self, mode: str, from_date: date, to_date: date) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if mode == "overall":
            ...
            return ads_df, revenue_df
        return (
            await self._read_ads_by_ad_type_with_range("user_acquisition", from_date, to_date),
            await self._read_first_deposit_revenue_with_range(from_date, to_date, ad_type="user_acquisition"),
        )

    async def cost_to_revenue_metrics_with_growth(self, mode: str, from_date: date, to_date: date) -> dict[str, object]:
        current_ads, current_revenue = await self._cost_to_revenue_frames(mode, from_date, to_date)
        previous_from, previous_to = self._previous_period_range(from_date, to_date)
        previous_ads, previous_revenue = await self._cost_to_revenue_frames(mode, previous_from, previous_to)
        current_cost = float(self._daily_totals_frame(current_ads, from_date, to_date)["cost"].sum())
        previous_cost = float(self._daily_totals_frame(previous_ads, previous_from, previous_to)["cost"].sum())
        current_revenue_idr = float(pd.to_numeric(current_revenue.get("revenue", 0), errors="coerce").fillna(0).sum()) * float(USD_TO_IDR_RATE)
        previous_revenue_idr = float(pd.to_numeric(previous_revenue.get("revenue", 0), errors="coerce").fillna(0).sum()) * float(USD_TO_IDR_RATE)
        current_metrics = {
            "cost_to_revenue_cost": round(current_cost, 2),
            "revenue": round(current_revenue_idr, 2),
            "cost_to_revenue_pct": round((current_revenue_idr / current_cost) * 100, 2) if current_cost else 0.0,
        }
        previous_metrics = {
            "cost_to_revenue_cost": round(previous_cost, 2),
            "revenue": round(previous_revenue_idr, 2),
            "cost_to_revenue_pct": round((previous_revenue_idr / previous_cost) * 100, 2) if previous_cost else 0.0,
        }
        growth = {metric: self._growth_percentage(float(current_metrics[metric]), float(previous_metrics[metric])) for metric in current_metrics.keys()}
        return {"current_period": {"from_date": from_date.isoformat(), "to_date": to_date.isoformat(), "metrics": current_metrics}, "previous_period": {"from_date": previous_from.isoformat(), "to_date": previous_to.isoformat(), "metrics": previous_metrics}, "growth_percentage": growth}

    async def cost_to_revenue_mode_payload(self, mode: str, from_date: date, to_date: date) -> dict[str, object]:
        metrics, chart = await asyncio.gather(
            self.cost_to_revenue_metrics_with_growth(mode=mode, from_date=from_date, to_date=to_date),
            self.cost_to_revenue_chart(from_date=from_date, to_date=to_date, mode=mode),
        )
    ...
    async def cost_to_revenue_chart(self, from_date: date, to_date: date, mode: str = "user_acquisition") -> dict[str, object]:
        ...
        daily["revenue_idr"] = daily["revenue"] * float(USD_TO_IDR_RATE)
        daily["first_deposit_idr"] = daily["revenue_idr"]
        daily["cost_to_revenue_pct"] = daily.apply(lambda row: round((float(row["revenue_idr"]) / float(row["cost"])) * 100, 2) if float(row["cost"]) else 0.0, axis=1)
        figure = go.Figure()
        ...
        figure.add_trace(go.Bar(x=date_labels, y=pd.to_numeric(daily["revenue_idr"], errors="coerce").fillna(0).tolist(), name="Revenue", marker_color="#13c39c", yaxis="y2", offsetgroup="deposit", hovertemplate="<b>%{x}</b><br>Revenue: Rp. %{y:,.0f}<extra></extra>"))
        ...
        chart_json = await asyncio.to_thread(json.dumps, figure, cls=plotly.utils.PlotlyJSONEncoder)
        rows = [{"date": row["date"].isoformat(), "cost": float(row["cost"]), "first_deposit_idr": float(row["first_deposit_idr"]), "revenue_idr": float(row["revenue_idr"]), "cost_to_revenue_pct": float(row["cost_to_revenue_pct"])} for _, row in daily.iterrows()]
        return {"rows": rows, "figure": json.loads(chart_json)}
