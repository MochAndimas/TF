...
class OverviewLeadsAcquisitionData:
    ...
    async def cost_to_revenue_chart(self, from_date: date, to_date: date) -> dict[str, object]:
        ...
        figure.add_trace(go.Bar(x=date_labels, y=pd.to_numeric(daily["first_deposit_idr"], errors="coerce").fillna(0).tolist(), name="First Deposit", marker_color="#13c39c", yaxis="y2", offsetgroup="deposit", hovertemplate="<b>%{x}</b><br>First Deposit: Rp. %{y:,.0f}<extra></extra>"))
        figure.add_trace(go.Scatter(x=date_labels, y=pd.to_numeric(daily["cost_to_revenue_pct"], errors="coerce").fillna(0).tolist(), mode="lines+markers", name="Cost To First Deposit", yaxis="y3", line=dict(color="#ff6248", width=2), hovertemplate="<b>%{x}</b><br>Cost To First Deposit: %{y:.2f}%<extra></extra>"))
        figure.update_layout(title="Cost To First Deposit Per Hari", barmode="group", xaxis=dict(type="category"), yaxis=dict(title="Cost"), yaxis2=dict(title="First Deposit", overlaying="y", side="right", anchor="free", position=0.94, showgrid=False), yaxis3=dict(title="Cost To First Deposit", overlaying="y", side="right", anchor="free", position=1, ticksuffix="%", showgrid=False), legend=dict(orientation="h", y=1.12, x=0))
        chart_json = await asyncio.to_thread(json.dumps, figure, cls=plotly.utils.PlotlyJSONEncoder)
