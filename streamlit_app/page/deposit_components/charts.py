...
def build_daily_deposit_amount_figure(report: dict[str, object], currency_unit: str) -> go.Figure:
    ...
    if not timeline or total_section is None:
        figure = go.Figure()
        figure.update_layout(title="Daily First Deposit Amount: New vs Existing", annotations=[{"text": "No data available", "xref": "paper", "yref": "paper", "x": 0.5, "y": 0.5, "showarrow": False}])
        return figure
    ...
    figure.update_layout(title="Daily First Deposit Amount: New vs Existing", barmode="stack", xaxis=dict(type="category"), yaxis=dict(title=f"First Deposit Amount ({currency_symbol})"), legend=dict(orientation="h", y=1.08, x=0), margin=dict(l=24, r=24, t=60, b=24))
    return figure
...
def build_daily_deposit_qty_aov_figure(report: dict[str, object], currency_unit: str) -> go.Figure:
    ...
    if not timeline or total_section is None:
        figure = go.Figure()
        figure.update_layout(title="Daily First Deposit Qty + AOV", annotations=[{"text": "No data available", "xref": "paper", "yref": "paper", "x": 0.5, "y": 0.5, "showarrow": False}])
        return figure
    ...
    figure.update_layout(title="Daily First Deposit Qty + AOV", xaxis=dict(type="category"), yaxis=dict(title="First Deposit Qty"), yaxis2=dict(title=f"AOV ({currency_symbol})", overlaying="y", side="right"), legend=dict(orientation="h", y=1.12, x=0), margin=dict(l=24, r=24, t=68, b=24))
    return figure
...
def build_top_campaign_deposit_figure(report: dict[str, object], currency_unit: str, top_n: int = 10) -> go.Figure:
    ...
    if not timeline or not campaign_sections:
        figure = go.Figure()
        figure.update_layout(title="Top Campaign by First Deposit Amount", annotations=[{"text": "No data available", "xref": "paper", "yref": "paper", "x": 0.5, "y": 0.5, "showarrow": False}])
        return figure
    ...
    if not rows:
        figure = go.Figure()
        figure.update_layout(title="Top Campaign by First Deposit Amount", annotations=[{"text": "No positive first deposit data", "xref": "paper", "yref": "paper", "x": 0.5, "y": 0.5, "showarrow": False}])
        return figure
    ...
    figure.update_layout(title="Top Campaign by First Deposit Amount", barmode="stack", xaxis=dict(title=f"First Deposit Amount ({currency_symbol})"), yaxis=dict(title="Campaign"), legend=dict(orientation="h", y=1.08, x=0), margin=dict(l=24, r=24, t=60, b=24))
    return figure
