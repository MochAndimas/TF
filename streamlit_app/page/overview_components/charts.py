...
def build_cost_vs_deposit_figure(rows: list[dict], currency_unit: str) -> go.Figure:
    ...
    if daily_df.empty:
        figure = go.Figure()
        figure.update_layout(title="Cost vs First Deposit Per Hari", annotations=[{"text": "No data available", "xref": "paper", "yref": "paper", "x": 0.5, "y": 0.5, "showarrow": False}])
        return figure
    ...
    figure.update_layout(title="Cost vs First Deposit Per Hari", barmode="group", xaxis=dict(type="category"), yaxis=dict(title=f"Cost ({currency_unit})"), yaxis2=dict(title=f"First Deposit ({currency_unit})", overlaying="y", side="right", showgrid=False), legend=dict(orientation="h", y=1.1, x=0))
    return figure
...
def build_cost_to_deposit_ratio_figure(rows: list[dict]) -> go.Figure:
    ...
    if daily_df.empty:
        figure = go.Figure()
        figure.update_layout(title="Cost To First Deposit (%) Per Hari", annotations=[{"text": "No data available", "xref": "paper", "yref": "paper", "x": 0.5, "y": 0.5, "showarrow": False}])
        return figure
    daily_df["date"] = pd.to_datetime(daily_df["date"], errors="coerce")
    daily_df["cost_to_revenue_pct"] = pd.to_numeric(daily_df.get("cost_to_revenue_pct", 0), errors="coerce").fillna(0.0)
    figure = go.Figure()
    figure.add_trace(go.Scatter(x=daily_df["date"].dt.strftime("%b %d\n%Y").tolist(), y=daily_df["cost_to_revenue_pct"].tolist(), mode="lines+markers", name="Cost To First Deposit", line=dict(color="#ff6248", width=2), hovertemplate="<b>%{x}</b><br>Cost To First Deposit: %{y:.2f}%<extra></extra>"))
    figure.update_layout(title="Cost To First Deposit (%) Per Hari", xaxis=dict(type="category"), yaxis=dict(title="Percent", ticksuffix="%"), legend=dict(orientation="h", y=1.1, x=0))
    return figure
