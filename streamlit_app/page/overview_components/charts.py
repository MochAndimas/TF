...
def build_cost_vs_deposit_figure(rows: list[dict], currency_unit: str, revenue_label: str = "Revenue") -> go.Figure:
    ...
    if currency_unit == "USD":
        ...
        deposit_hover = f"<b>%{{x}}</b><br>{revenue_label}: $ %{{y:,.2f}}<extra></extra>"
        cost_name = "Cost (USD)"
        deposit_name = f"{revenue_label} (USD)"
    else:
        cost_hover = "<b>%{x}</b><br>Cost: Rp %{y:,.0f}<extra></extra>"
        deposit_hover = f"<b>%{{x}}</b><br>{revenue_label}: Rp %{{y:,.0f}}<extra></extra>"
        cost_name = "Cost (IDR)"
        deposit_name = f"{revenue_label} (IDR)"
    figure = go.Figure()
    ...
    figure.update_layout(title=f"Cost vs {revenue_label} Per Hari", barmode="group", xaxis=dict(type="category"), yaxis=dict(title=f"Cost ({currency_unit})"), yaxis2=dict(title=f"{revenue_label} ({currency_unit})", overlaying="y", side="right", showgrid=False), legend=dict(orientation="h", y=1.1, x=0))
    return figure
