"""Streamlit page for login activity analytics."""
...
def _render_filters() -> tuple[dt.date | None, dt.date | None, str | None]:
    ...
    st.markdown('<div class="campaign-title">Login</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        with period_col:
            ...
            if selected_period == "Custom Range":
                ...
                if st.session_state.get("login_activity_date_range") != (start_date, end_date):
                    st.session_state["login_activity_date_range"] = (start_date, end_date)
        with source_col:
            selected_source = st.selectbox("Login Source", options=list(SOURCE_OPTIONS.keys()), index=0, key="login_activity_source")
...
def _render_metrics(metrics: dict[str, object]) -> None:
    ...
    metric_specs = [
        ("Total Login", "total_login", _fmt_int(current_metrics.get("total_login")), None),
        ("Avg Daily", "avg_daily_login", _fmt_float(current_metrics.get("avg_daily_login")), None),
        ...
        ("Peak Day Login", "peak_day_login", _fmt_int(current_metrics.get("peak_day_login")), f"Peak day: {current_metrics.get('peak_day') or '-'}"),
    ]
...
def _details_dataframe(rows: list[dict[str, object]]) -> pd.DataFrame:
    ...
    rename_map = {
        ...
        "avg_daily_login": "Avg Daily",
        "share_pct": "Share %",
        "first_date": "First Date",
        "last_date": "Last Date",
    }
    df = df.rename(columns=rename_map)
    columns = ["Campaign ID", "Campaign Name", "Source", "Type", "Login", "Share %", "Avg Daily"]
    return df[[column for column in columns if column in df.columns]]


async def show_login_activity_page(host: str) -> None:
    ...
    data = st.session_state.get("login_activity_payload", {}).get("data", {})
    _render_metrics(data.get("metrics", {}))
    charts = data.get("charts", {})
    daily_figure = set_transparent_chart_background(campaign_figure_from_payload(charts.get("daily_trend", {}).get("figure"), "Daily Login"))
    cumulative_figure = set_transparent_chart_background(campaign_figure_from_payload(charts.get("cumulative_trend", {}).get("figure"), "Cumulative Login by Campaign"))
    source_figure = set_transparent_chart_background(campaign_figure_from_payload(charts.get("source_mix", {}).get("figure"), "Login by Source"))
    type_figure = set_transparent_chart_background(campaign_figure_from_payload(charts.get("type_mix", {}).get("figure"), "Login by Campaign Type"))
    top_figure = set_transparent_chart_background(campaign_figure_from_payload(charts.get("top_campaigns", {}).get("figure"), "Top Campaigns by Login"))
    heatmap_figure = set_transparent_chart_background(campaign_figure_from_payload(charts.get("campaign_heatmap", {}).get("figure"), "Daily Login Heatmap"))
    ...
    st.markdown("### Campaign Login Details")
    details_df = _details_dataframe(data.get("details", []))
    if details_df.empty:
        st.info("No login activity data for selected date range.")
        return
    st.dataframe(
        details_df,
        width="stretch",
