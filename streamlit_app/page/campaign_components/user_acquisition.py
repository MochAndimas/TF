...
def build_performance_dataframe(detail_rows: list[dict], level_column: str) -> pd.DataFrame:
    ...
    grouped["cost_per_leads"] = grouped.apply(lambda row: round(float(row["spend"]) / float(row["leads"])) if float(row["leads"]) else 0.0, axis=1)
    return grouped
...
def build_performance_table(detail_rows: list[dict]):
    """Build UA performance table data and return the active level selection."""
    level_options = {
        "Ad Campaign Performance": ("campaign_id", "Campaign ID"),
        "Ad group Performance": ("ad_group", "Ad Group"),
        "Ad Name Performance": ("ad_name", "Ad Name"),
    }
    selected_level = st.selectbox("Performance Table", options=list(level_options.keys()), key="campaign_performance_level")
...
def render_performance_table(level_label: str, display_df: pd.DataFrame) -> None:
    """Render the prepared UA performance table."""
